...
app.post("/checkout", async (req, res) => {
  try {
    ...
    // PayFast payment data (field ORDER matters — do NOT sort)
    const paymentData = {
    ...
    // ── Signature: encode with + for spaces (PayFast spec) ────────────────────
    const sigEncode = v =>
    ...
    // ── Redirect URL: standard encodeURIComponent for the browser ─────────────
    // PayFast receives these, parses them normally, then re-signs using + encoding
...
app.post("/webhook", async (req, res) => {
  // Always respond 200 immediately to satisfy PayFast timeout
